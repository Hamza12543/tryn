...
import {updateProduct, type ProductActionState} from "@/actions/product"
...
export function EditProductForm({product, productId}: EditProductFormProps) {
  const [state, formAction, isPending] = useActionState(
    (prevState: ProductActionState, formData: FormData) => updateProduct(productId, prevState, formData),
    {
      success: false,
      error: undefined,
  ...
  useEffect(() => {
    if (product) {
      ...
      setField("stock", product.stock?.toString() || "")
      // Don't set category here - wait for categories to load first
      setField("actualPrice", product.actualPrice?.toString() || "")
      setField("discountedPrice", product.discountedPrice?.toString() || "")
      setField("weight", product.weight?.toString() || "")
      setField("length", product.dimensions?.length?.toString() || "")
      setField("width", product.dimensions?.width?.toString() || "")
      setField("careInstructions", product.careInstructions || "")
      setField("warranty", product.warranty || "")
      setField("deliveryInfo", product.deliveryInfo || "")
      setField("returnPolicy", product.returnPolicy || "")
      setField("seoTitle", product.seoTitle || "")
      setField("seoDescription", product.seoDescription || "")
      setField("isActive", product.isActive || false)
      setField("isFeatured", product.isFeatured || false)
      setField("freeShipping", product.freeShipping || false)
      setField("inStock", product.inStock || false)
      setField("allowReviews", product.allowReviews || false)

      if (product.images && Array.isArray(product.images)) {
        setField("productImages", product.images)
      }
      if (product.bannerImages && Array.isArray(product.bannerImages)) {
        setField("bannerImages", product.bannerImages)
      }

      if (product.colors && Array.isArray(product.colors)) {
        setField("selectedColors", product.colors)
      }
      if (product.sizes && Array.isArray(product.sizes)) {
        setField("selectedSizes", product.sizes)
      }

      if (product.features && Array.isArray(product.features)) {
        setField("features", product.features)
      }
      if (product.specifications && Array.isArray(product.specifications)) {
        setField("specifications", product.specifications)
      }
    }
  }, [product, setField])
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Once categories are loaded, match with product category
        if (data.length > 0 && product?.category) {
          // Try to match by slug first, then by name as fallback
          let matchedCategory = data.find((cat: Category) => cat.slug === product.category)

          // If no match by slug, try matching by name
          if (!matchedCategory) {
            matchedCategory = data.find((cat: Category) => cat.name === product.category)
          }

          if (matchedCategory) {
            setField("selectedCategory", matchedCategory.slug)
          }
        }
      } catch (error) {
    ...
    if (files) {
      ...
      for (let i = 0; i < files.length; i++) {
        try {
          ...
          if (file.size > 1024 * 1024) {
            try {
              processedFile = await compressImage(file, 1200, 300, 0.7)
            } catch (compressError) {
  ...
  const handleSubmit = async () => {
    ...
    submitFormData.append("dimensions.height", "")
    ...
    submitFormData.append("hasDiscount", (discountedPrice && discountedPrice.trim() !== "").toString())
    submitFormData.append("freeShipping", freeShipping.toString())
    submitFormData.append("inStock", (parseFloat(stock) > 0).toString())
